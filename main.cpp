...
char nodes_file[MAX_STRING], users_file[MAX_STRING], hin_file[MAX_STRING], output_file[MAX_STRING];//, output_file2[MAX_STRING];
...
void TrainModel() {
	...
	words.init(users_file, vector_size);
...
int main(int argc, char **argv) {
	...
	if ((i = ArgPos((char *)"-users", argc, argv)) > 0) strcpy(users_file, argv[i + 1]);
